function tagfaces(args){
  ...
  return new Promise(function(resolve, reject) {
    Jimp.loadFont(Jimp.FONT_SANS_16_BLACK).then(function (font) {
      new Jimp(dimensions.width, dimensions.height, 0xD3D3D3, function (err, bg_img) {
          ...
          for(var k=0; k<images.length; k++){
            if (images[k].image.indexOf(faces[i].name) != -1 && images[k].faces.length == 1 && "identity" in images[k].faces[0]){
              ...
              Jimp.read(bitmap_face, function (err, img) {
       	        bg_img.composite(img, face.location.left, face.location.top);
      	        bg_img.print(font, face.location.left, face.location.top + img.bitmap.height, face_name);
                if(bg_img != null && i == faces.length - 1 && k == images.length - 1){
                  bg_img.getBuffer(Jimp.MIME_JPEG, function (err, img_buffer) {  
